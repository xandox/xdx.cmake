...
macro(xdx_project)
    ...
    if (NOT _xdx_TYPE)
        _xdx_fatal("Type not set.")
    endif()

    if (NOT _xdx_NAME)
        _xdx_fatal("Name not set.")
    endif()

    string(REPLACE "::" "/" _xdx_INCLUDE_SUFFIX "${_xdx_NAMESPACE}")
    ...
    elseif(_xdx_TYPE STREQUAL "interface")
        if (_xdx_sources_full)
            _xdx_fatal("Interface target '${_xdx_NAME}' has SOURCES.")
        endif()
        ...
        _xdx_fatal("Unknown project type: ${_xdx_TYPE}")
    endif()
    ...
    target_compile_features(${_xdx_NAME} PUBLIC ${XDX_CPP_STANDARD})
    ...
    if (XDX_ENABLE_TESTING)
        if (_xdx_tests_full)
            ...
            target_link_libraries(${_xdx_NAME}.tests PRIVATE GTest::GTest GTest::Main ${_xdx_NAME})
